...
parser.add_argument("--model_file", type=str, default = './test_seg/models.txt')
...
class Dataset:
	...
	def __init__(
		...
		self.masks_fps = [os.path.join(masks_dir, image_id+'.tif') for image_id in self.ids]
...
result_dir = os.path.join(model_folder,'evals')
...
# 	print(pr_masks.shape); print(gt_masks.shape)
	...
	def save_pr(pr_save_mask, index = 0):
		id_img = index
		ch = pr_save_mask.shape[-1]
		example_dir = model_folder + '/example'
		generate_folder(example_dir)
		for i in range(ch):
			imsave(example_dir+'/{}-ch-{}.png'.format(id_img, i), pr_save_mask[id_img,::4,::4,i]*1.)
	save_pr(gt_save_mask, index = 50)
	...
	print('iou_classes: {:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f}; mIoU: {:.4f}'.format(iou_classes[0],iou_classes[1],iou_classes[2], iou_classes[3],iou_classes[4], iou_classes[5], mIoU))
	print('dice_classes: {:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f}; mDice: {:.4f}'.format(dice_classes[0],dice_classes[1],dice_classes[2], dice_classes[3], dice_classes[4], dice_classes[5],mDice))
	# save the prediction
...
# 	save = True
	...
	print('Precision:{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f}'.format(prec_scores[0], prec_scores[1], prec_scores[2],prec_scores[3],prec_scores[4],prec_scores[5]))
	print('mean precision: {:.4f}\n'.format(np.mean(prec_scores)))
	print('Recall:{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f}'.format(recall_scores[0], recall_scores[1], recall_scores[2],recall_scores[3],recall_scores[4],recall_scores[5]))
	print('mean recall:{:.4f}\n'.format(np.mean(recall_scores)))
	# f1 score
	print('f1-score (pixel):{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f}'.format(f1_scores[0],f1_scores[1],f1_scores[2], f1_scores[3], f1_scores[4], f1_scores[5]))
	print('mean f1-score (pixel):{:.4f}\n'.format(np.mean(f1_scores)))

	with open(result_dir+'/{}_summary_{}.txt'.format(subset, epoch), 'w+') as f:
		# save iou and dice
		f.write('iou_classes: {:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f}; mIoU: {:.4f}\n'.format(iou_classes[0],iou_classes[1],iou_classes[2], iou_classes[3],iou_classes[4], iou_classes[5], mIoU))
		f.write('dice_classes: {:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f}; mDice: {:.4f}\n'.format(dice_classes[0],dice_classes[1],dice_classes[2], dice_classes[3], dice_classes[4], dice_classes[5],mDice))
		# save confusion matrix
		f.write('confusion matrix:\n')
		np.savetxt(f, cf_mat, fmt='%-7d')
		# save precision
		f.write('Precision:{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f}\n'.format(prec_scores[0], prec_scores[1], prec_scores[2],prec_scores[3],prec_scores[4],prec_scores[5]))
		f.write('mean precision: {:.4f}\n'.format(np.mean(prec_scores)))
		# save recall
		f.write('Recall:{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f}\n'.format(recall_scores[0], recall_scores[1], recall_scores[2],recall_scores[3],recall_scores[4],recall_scores[5]))
		f.write('mean recall:{:.4f}\n'.format(np.mean(recall_scores)))
		# save f1-score
		f.write('f1-score (pixel):{:.4f},{:.4f},{:.4f},{:.4f},{:.4f},{:.4f}\n'.format(f1_scores[0],f1_scores[1],f1_scores[2], f1_scores[3], f1_scores[4], f1_scores[5]))

...
image_dir = DATA_DIR+'/{}'.format('images') if not crop else DATA_DIR+'/crop/{}'.format('images')
masks_dir = DATA_DIR+'/{}'.format('seg_maps') if not crop else DATA_DIR+'/crop/{}'.format('seg_maps')
...
# 	gt_save_mask = np.concatenate([gt_masks[:,:,:,-1:],gt_masks[:,:,:,:-1]], axis = -1)
	pr_save_mask = pr_masks
	gt_save_mask = gt_masks
	...
	# print('iou_classes: {:.4f},{:.4f}; mIoU: {:.4f}'.format(iou_classes[0],iou_classes[1], mIoU))
	# print('dice_classes: {:.4f},{:.4f}; mDice: {:.4f}'.format(dice_classes[0],dice_classes[1],mDice))
	# save the prediction
	pr_save_map = 1.0* (pr_save_mask > 0.5)
	gt_save_map = (gt_save_mask > 0.5) * 1.0
# 	save = True
	save = args.save
	if save:

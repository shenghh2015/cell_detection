...
from btstrp_models import cv_models, non_cv_models
...
def main():
		...
		for model_name in model_names:
				...
				for model_path in weight_files:
						...
						mAP = evaluate_ap(model, phi, dataset, cross = bstrp, cls = cls)
						mAP_list.append(mAP)
				best_index = np.argmax(mAP_list); print('Best mAP: {:.3f}'.format(mAP_list[best_index]))
				model_path = weight_files[best_index]
				model_dir = os.path.dirname(model_path)
				model.load_weights(model_path, by_name=True)
				evaluate_ap(model, phi, dataset, model_path, cross = bstrp, cls = cls, save = True)
				gt_boxes = load_test_boxes(dataset, cross = bstrp, cls = cls)
